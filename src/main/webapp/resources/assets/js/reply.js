...
let replyService = (function(){
   ...
   function getList(data,callback,error){
      ...
      $.getJSON(
         "/moim/replypages/"+boardnum+"/"+page+".json", //ReplyController
         //두번째로 넘겨주는 함수의 매개변수는 getJSON으로 받아온 JSON을 담는다.
         function(data){
            //data : ReplyPageDTO를 JSON으로 바꾼것
            if(callback){
               //성공시 호출할 함수를 넘겨 받았다면 그거 호출!
               //Ajax 요청으로 받아온 데이터의 replyCnt와 list를 넘겨주면서 호출한다.
               //흐름이 get.jsp의 159번째 줄로 이동
   ...
   function update(reply,callback,error){
      $.ajax({
         type:"PUT",
         url:"/moim/updatereply/"+reply.replynum,
         data:JSON.stringify(reply),
         contentType:"application/json; charset=utf-8",
         success:function(result,status,xhr){
            if(callback){
   ...
   function displayTime(time){
      ...
      //데이트 객체 .getTime() : 현재 시간 정보를 밀리초로 변환
