...
let shopReplyService = (function(){
	...
	function getList(data,callback,error){
		//2323
		let shopNum = data.shopNum;
		//2
		let page = data.page;
		// /reply/pages/2323/2.json
		$.getJSON(
			"/sreply/pages/"+shopNum+"/"+page+".json", //ReplyController로 이동
	...
	//댓글 삭제
	function remove(SReplyNum,callback,error){
		$.ajax({
			type:"DELETE",
			url:"/sreply/"+SReplyNum,
			success:function(result,status,xhr){
				if(callback){
					callback(result);
				}
	...
	//댓글 수정
	function update(sreply,callback,error){
			console.log("putputput");
		$.ajax({
			type:"PUT",
			url:"/sreply/"+sreply.SReplyNum,
			data:JSON.stringify(sreply),
			contentType:"application/json;charset=utf-8",
			success:function(result,status,xhr){
				if(callback){
	...
	//댓글 조회
	function get(SReplyNum,callback){
		$.ajax({
			type:"GET",
			url:"/sreply/"+SReplyNum,
			success:function(result,status,xhr){
