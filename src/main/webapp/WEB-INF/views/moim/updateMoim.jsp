...
<body class="is-preload">
	...
	<div id="main">
		<div class="wrapper">
			<div class="inner">
				<hr>
				<form name="updateform" action="/moim/updateMoim" method="post">
					...
					<div id="container">
						...
						<table>
							...
							<tr>
								<td>모임 이름</td>
								<td colspan="2"><input type="text" name="moimname" value="${moim.moimname}"></td>
							</tr>
							<tr>
								<td>여행 분류</td>
								<td colspan="2"><select name="moimtype">

										<c:choose>
											<c:when test="${moim.moimtype == 0 }">
												<option value="0" selected>일반</option>
												<option value="1">카라반</option>
												<option value="2">바이크</option>

											</c:when>
											<c:when test="${moim.moimtype == 1 }">
												<option value="0">일반</option>
												<option value="1" selected>카라반</option>
												<option value="2">바이크</option>

											</c:when>
											<c:when test="${moim.moimtype == 2 }">
												<option value="0">일반</option>
												<option value="1">카라반</option>
												<option value="2" selected>바이크</option>
								...
								<td style="width: 80%;">
									<div class="map_wrap">
										...
										<div id="menu_wrap" class="bg_white">
											<div class="option">
												<div>
													<input type="text" value="${place.placename}" id="keyword" size="15">
													<input type="button" onclick="searchPlaces(); return false;" />
												</div>
											</div>
											<hr>
											<ul id="placesList"></ul>
									...
									<div>
										<input type="text" id="placename" name="placename"
											value="${place.placename}" readonly>
										<input
											type="hidden" id="placelat" name="placelat"
											value="${place.placelat}">
										<input type="hidden"
											id="placelon" name="placelon" value="${place.placelon}">
										<input type="hidden" id="placeaddr" name="placeaddr"
											value="${place.placeaddr}">
										<input type="hidden"
											id="placeroadaddr" name="placeroadaddr"
											value="${place.placeroadaddr}">
										<input type="hidden"
											id="placecategory" name="placecategory"
											value="${place.placecategory}">
										<input type="hidden"
											id="placephonenumber" name="placephonenumber"
											value="${place.placephonenumber}">
									</div>
								</td>
							</tr>
							<tr>
								<td>경비</td>
								<td colspan="2">
									<input type="text" name="moimbudget" value="${moim.moimbudget}"></td>
							</tr>
							<tr>
								<c:set var="startdate" value="${fn:split(fn:split(moim.startdate,' ')[0],'-')}"></c:set>
								<td>출발날짜</td>
								<td colspan="2">
									<input type="text" id="datepicker1" name="startdate"
									value="${startdate[1]}/${startdate[2]}/${startdate[0]}">
								</td>
							</tr>
							<tr>
								<c:set var="enddate" value="${fn:split(fn:split(moim.enddate,' ')[0],'-')}"></c:set>
								<td>끝날짜</td>
								<td colspan="2">
									<input type="text" id="datepicker2" name="enddate"
									value="${enddate[1]}/${enddate[2]}/${enddate[0]}">
								</td>
							</tr>
							<tr>
								<td>모임장</td>
								<td colspan="2"><input type="text" name="userid" value="${moim.userid}" readonly></td>
							</tr>
							<tr>
								<td colspan="3" class="put">
									<input type="submit" value="수정 완료"></td>
								<td colspan="3" class="put">
									<input type="button" value="돌아가기" onclick="location.href='/moim/get/'+${moim.moimnum}"></td>
							</tr>
						</table>
					</div>
				</form>
...
<script src="/resources/assets/js/map1.js"></script>
