			...
			<div class="inner">
				...
				<div class="table-wrapper">
					<table style="table-layout: fixed; border-spacing: 2px">
						...
						<tbody>
							<c:choose>
								<c:when test="${members != null and members.size()>1}">
									<script>
                        	console.log("${members}")
                        </script>
									<c:forEach items="${members}" var="member">
										<c:if test="${'banana'!=member.userId}">
											<tr style="text-align: center;">
												<td style="width: 20%">${member.userId}</td>
												<td style="width: 20%">${member.userName}</td>
												<td style="width: 20%">${member.userNickname}</td>
												<td style="width: 20%">${member.userPhone}</td>
												<td style="width: 20%">${member.userEmail}</td>
												<td style="width: 20%">
													<a class="fire button primary"
													href="${member.userId}&${moimnum}">쫒아내기</a>
												</td>
											</tr>
										</c:if>
									</c:forEach>
...
/*       frm.append("<input type='hidden' name='moimjoinnum' value='"+$(this).attr("href")+"'>")
   ...
   $(".fire").on("click",function(e){
      ...
      let joindetail = window.open("/moim/fireMessage?userId="+splitHref[0]+"&moimnum="+splitHref[1],"fireMessage","width=1000,height=1000,left=0,right=0")
